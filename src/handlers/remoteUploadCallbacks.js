...
function downloadFileContent(fileUrl, redirectCount = 0) {
...
async function uploadSingleFileToByse(telegram, chatId, file, ruState) {
  const apiKey = getApiKey();
  if (!apiKey) {
    await telegram.sendMessage(chatId,
      '❌ `BYSE_API_KEY` belum diset — tidak bisa upload ke Byse.',
      { parse_mode: 'Markdown' }
    );
  ...
  if (existing) {
    ...
    const fileUrl = existing.link || '';
    await telegram.sendMessage(chatId,
      `⏭️ *File sudah pernah diupload ke Byse*\n` +
      `📹 \`${shortName}\`\n` +
      `🔑 \`${existing.filecode}\`` +
      (fileUrl ? `\n🔗 ${fileUrl}` : ''),
      { parse_mode: 'Markdown' }
    ...
    return { ok: true, filecode: existing.filecode, link: fileUrl, skipped: true };
  }
  ...
  try {
    ...
    if (res.ok) {
      ...
      if (ruState) ruState.addJob({ filecode, url: `local:${file.name}` });

      let activeFilecode = filecode;

      const fldId = await getOrCreateFolder(apiKey, folderName).catch(() => null);
      if (fldId) {
        const moveRes = await setFileFolder(apiKey, filecode, fldId).catch(() => ({ ok: false }));
        if (moveRes.ok) {
          activeFilecode = moveRes.filecode || filecode;
          if (ruState && activeFilecode !== filecode) {
            ruState.updateJobFilecode(filecode, activeFilecode);
          }
        }
      }

      const fileUrl = await getByseFileUrl(apiKey, activeFilecode);

      uploadHistory.add({
        filename: file.name,
        sizeBytes: file.size,
        filecode: activeFilecode,
        link: fileUrl,
        folder: folderName !== BYSE_BASE_DIR ? folderName : ''
      });
      ...
      await editStatus(
        `✅ *Upload ke Byse selesai!*\n` +
        `📹 \`${shortName}\`\n` +
        `🔑 \`${activeFilecode}\`` +
        (fileUrl ? `\n🔗 ${fileUrl}` : '') +
        `\n🗑️ _File lokal sudah dihapus_`
      );

      logger.upload.done(file.name, activeFilecode);
      return { ok: true, filecode: activeFilecode, link: fileUrl };
    } else {
      await editStatus(`❌ *Upload gagal*\n📹 \`${shortName}\`\n_${res.error}_`);
      logger.upload.fail(file.name, res.error);
      return { ok: false, error: res.error };
    }
  } catch (e) {
    await editStatus(`❌ *Upload error*\n📹 \`${shortName}\`\n_${e.message}_`);
...
function registerRemoteUploadHandlers(bot, remoteUploadState) {
  ...
  bot.action('remote_upload_menu', async ctx => {
    try {
      ...
      if (!e.message?.includes('message is not modified')) {
        logger.error('remote_upload_menu error', { error: e.message });
      }
    }
  });
    ...
    for (const file of files) {
      ...
      try {
        ...
        if (res.ok) {
          ...
          ruState.addJob({ filecode, url: `local:${file.name}` });

          let activeFilecode = filecode;

          const fldId = await getOrCreateFolder(apiKey, folderName).catch(e => {
            logger.warn('Gagal dapatkan/buat folder Byse', { folderName, error: e.message });
            return null;
          });
          if (fldId) {
            const moveRes = await setFileFolder(apiKey, filecode, fldId).catch(e => {
              logger.warn('Gagal pindah file ke folder Byse', { filecode, fldId, error: e.message });
              return { ok: false };
            });
            if (moveRes.ok) {
              activeFilecode = moveRes.filecode || filecode;
              if (activeFilecode !== filecode) {
                ruState.updateJobFilecode(filecode, activeFilecode);
              }
              logger.info('File dipindah ke folder Byse', { filecode, newFilecode: activeFilecode, folderName, fldId });
            }
          }
  ...
  async function showExplorer(ctx, page) {
    const apiKey = getApiKey();
    if (!apiKey) {
      await ctx.editMessageText('❌ `BYSE_API_KEY` belum diset.', { parse_mode: 'Markdown', ...getBackKeyboardTo('remote_upload_menu') });
      return;
    ...
    const [foldersRes, accountRes] = await Promise.all([
      listFoldersFull(apiKey),
      getAccountInfo(apiKey).catch(() => null),
    ]);

    const folders = foldersRes.ok ? foldersRes.folders : [];
    const accountLine = (accountRes && accountRes.ok)
      ? `📧 *${accountRes.data.email || ''}*  |  💾 ${formatBytes(parseInt(accountRes.data.storage_used || 0))}\n\n`
      : '';

    const totalPages = Math.max(1, Math.ceil(folders.length / EXPLORER_FOLDERS_PER_PAGE));
    const safePage = Math.min(Math.max(page, 1), totalPages);
    const start = (safePage - 1) * EXPLORER_FOLDERS_PER_PAGE;
    const pageFolders = folders.slice(start, start + EXPLORER_FOLDERS_PER_PAGE);

    const folderLines = pageFolders.length > 0
      ? pageFolders.map(f => {
          const count = f.file_count !== undefined ? ` — ${f.file_count} file` : '';
          return `📁 \`${(f.name || 'Unnamed').slice(0, 35)}\`${count}`;
        }).join('\n')
      : '_Belum ada folder_';

    const pageInfo = totalPages > 1 ? ` _(hal. ${safePage}/${totalPages})_` : '';

    await ctx.editMessageText(
      `🔍 *Browse Byse*${pageInfo}\n` +
      `━━━━━━━━━━━━━━━━━━━━\n\n` +
      accountLine +
      `*Folder:*\n${folderLines}\n\n` +
      `Pilih folder untuk melihat isi file:`,
      { parse_mode: 'Markdown', ...getByseExplorerKeyboard(folders, safePage) }
    );
  }

  bot.action('byse_explorer', async ctx => {
    try {
      await ctx.answerCbQuery('Memuat daftar folder...');
      await showExplorer(ctx, 1);
    } catch (e) {
      logger.error('byse_explorer error', { error: e.message });
    }
  });

  bot.action(/^byse_explorer\|(\d+)$/, async ctx => {
    try {
      await ctx.answerCbQuery('Memuat...');
      await showExplorer(ctx, parseInt(ctx.match[1]) || 1);
    } catch (e) {
      logger.error('byse_explorer page error', { error: e.message });
    }
  });

  bot.action(/^bex_folder\|(\d+)\|(\d+)$/, async ctx => {
    try {
      const fld_id = parseInt(ctx.match[1], 10);
      const page = parseInt(ctx.match[2], 10);
      await ctx.answerCbQuery('Memuat file...');
      ...
      const PER_PAGE = 15;
      const res = await listFiles(apiKey, fld_id, page, PER_PAGE);

      if (!res.ok) {
        await ctx.editMessageText(
          `❌ Gagal memuat file: _${res.error}_`,
          { parse_mode: 'Markdown', ...getBackKeyboardTo('byse_explorer') }
        );
        return;
      }
      ...
      const folderLabel = fld_id === 0 ? '🗂️ Root' : `📁 Folder #${fld_id}`;

      if (files.length === 0) {
        await ctx.editMessageText(
          `${folderLabel}\n\n_Tidak ada file di sini._`,
          { parse_mode: 'Markdown', ...getByseFilesKeyboard(files, fld_id, page, totalPages) }
        );
        return;
      }

      const getLink = (code) => `https://byse.sx/e/${code}`;

      const nameCounts = {};
      files.forEach(f => {
        const n = (f.name || f.title || '').toLowerCase().trim();
      ...
      const lines = files.map((f, i) => {
        const name = escapeMd((f.name || f.title || f.file_code || 'Unknown').slice(0, 40));
        const code = f.file_code || f.filecode || '';
        const views = f.views !== undefined ? ` 👁️${f.views}` : '';
        const canplay = f.canplay === 1 || f.canplay === '1' ? '▶️' : '⏳';
        const isDupe = nameCounts[(f.name || f.title || '').toLowerCase().trim()] > 1;
        const dupeFlag = isDupe ? ' ⚠️ _DUPLIKAT_' : '';
        const num = (page - 1) * PER_PAGE + i + 1;
        return `${num}\\. ${canplay} \`${code}\`${dupeFlag}\n   📄 ${name}\n   🔗 ${getLink(code)}${views}`;
      });

      const header =
        `${folderLabel} — Halaman ${page}/${totalPages}\n` +
        `━━━━━━━━━━━━━━━━━━━━\n` +
        `📊 Total: *${total}* file\n\n`;
      ...
      for (const line of lines) {
        if (chunk.length + line.length + 2 > 3800) {
      ...
      try {
        await ctx.editMessageText(bodyChunks[0], { parse_mode: 'Markdown', ...getByseFilesKeyboard(files, fld_id, page, totalPages) });
        for (let i = 1; i < bodyChunks.length; i++) {
          await ctx.reply(bodyChunks[i], { parse_mode: 'Markdown' });
        }
      } catch (e) {
        await ctx.reply(bodyChunks[0], { parse_mode: 'Markdown', ...getByseFilesKeyboard(files, fld_id, page, totalPages) });
      }
    } catch (e) {
      logger.error('bex_folder error', { error: e.message });
    }
  });

  bot.action('bex_dupes', async ctx => {
    try {
      await ctx.answerCbQuery('Memindai duplikat...');
      const apiKey = getApiKey();
      if (!apiKey) {
        await ctx.editMessageText('❌ `BYSE_API_KEY` belum diset.', { parse_mode: 'Markdown', ...getBackKeyboardTo('byse_explorer') });
        return;
      }

      await ctx.editMessageText(
        '🔍 *Scan Duplikat Byse*\n\n⏳ Sedang mengambil data dari semua folder...',
        { parse_mode: 'Markdown' }
      );

      const foldersRes = await listFoldersFull(apiKey);
      const folders = foldersRes.ok ? foldersRes.folders : [];

      async function fetchAllFilesInFolder(fld_id, label) {
        const collected = [];
        const PER_PAGE = 200;
        let page = 1;
        while (true) {
          const res = await listFiles(apiKey, fld_id, page, PER_PAGE);
          if (!res.ok || res.files.length === 0) break;
          res.files.forEach(f => collected.push({ ...f, _location: label }));
          if (res.files.length < PER_PAGE) break;
          page++;
        }
        return collected;
      }
      ...
      const rootFiles = await fetchAllFilesInFolder(0, 'Root');
      allFiles.push(...rootFiles);

      for (const folder of folders) {
        const label = folder.name || `Folder #${folder.fld_id}`;
        const folderFiles = await fetchAllFilesInFolder(folder.fld_id, label);
      ...
      if (dupes.length === 0) {
        await ctx.editMessageText(
          `🔍 *Scan Duplikat Selesai*\n\n✅ Tidak ditemukan duplikat dari ${allFiles.length} file yang dipindai.`,
          { parse_mode: 'Markdown', ...getBackKeyboardTo('byse_explorer') }
        );
        return;
      }
      ...
      let report = `🔍 *Duplikat Ditemukan: ${dupes.length} nama*\n` +
        `━━━━━━━━━━━━━━━━━━━━\n` +
        `📊 Dipindai: *${allFiles.length}* file\n\n`;

      for (const [name, arr] of dupes.slice(0, 15)) {
        const safeName = name.slice(0, 45);
        report += `⚠️ *${safeName}*\n`;
        arr.forEach(f => {
          const code = f.file_code || f.filecode || '?';
          report += `   • \`${code}\` _[${f._location}]_ ${getLink(code)}\n`;
        });
        report += '\n';
      }

      if (dupes.length > 15) {
        report += `_...dan ${dupes.length - 15} nama duplikat lainnya_\n`;
      }

      report += `\n💡 _Hapus duplikat manual di website Byse._`;
      ...
      if (report.length > 4000) {
        ...
        parts.push(part);
        await ctx.editMessageText(parts[0], { parse_mode: 'Markdown', ...getBackKeyboardTo('byse_explorer') });
        for (let i = 1; i < parts.length; i++) {
          await ctx.reply(parts[i], { parse_mode: 'Markdown' });
        }
      } else {
        await ctx.editMessageText(report, { parse_mode: 'Markdown', ...getBackKeyboardTo('byse_explorer') });
      }
    } catch (e) {
      logger.error('bex_dupes error', { error: e.message });
    }
  });
  ...
  const handleDocument = async (ctx) => {
    ...
    for (let i = 0; i < urls.length; i++) {
      ...
      if (remoteUploadState.hasUrl(url)) {
        skippedDupes++;
        const shortUrl = url.length > 45 ? url.slice(0, 45) + '…' : url;
        results.push(`⏭️ \`${shortUrl}\`\n   _(URL sudah ada di antrian)_`);
        continue;
      }
    ...
    const summary =
      ...
      `━━━━━━━━━━━━━━━━━━━━\n\n` +
      `✅ Berhasil: *${success}*   ❌ Gagal: *${failed}*   ⏭️ Duplikat: *${skippedDupes}*\n\n` +
      results.slice(0, 20).join('\n') +
      (results.length > 20 ? `\n_...dan ${results.length - 20} lainnya_` : '') +
...
module.exports = { registerRemoteUploadHandlers, uploadSingleFileToByse };

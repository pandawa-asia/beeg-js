...
async function setFileFolder(apiKey, filecode, fld_id, retries = 3, delayMs = 5000) {
  for (let attempt = 1; attempt <= retries; attempt++) {
    try {
      ...
      if (res.status === 200) {
        const newFilecode = (res.result && (res.result.file_code || res.result.filecode)) || filecode;
        logger.info('Byse file dipindah ke folder', { filecode, fld_id, newFilecode });
        return { ok: true, filecode: newFilecode };
      }
...
async function listFiles(apiKey, fld_id = 0, page = 1, per_page = 20) {
  try {
    const res = await httpGet(
      `${BYSE_BASE}/file/list?key=${encodeURIComponent(apiKey)}&fld_id=${fld_id}&per_page=${per_page}&page=${page}`
    );
    if (res.status === 200 && res.result) {
      const r = res.result;
      const files = r.results || r.files || r || [];
      return {
        ok: true,
        files: Array.isArray(files) ? files : [],
        total: r.total_files || r.total || 0,
        page: r.current_page || page,
        limit: r.limit || per_page,
      };
    }
    return { ok: false, error: res.msg || 'Gagal list file', files: [], total: 0 };
  } catch (e) {
    logger.error('Byse listFiles error', { error: e.message });
    return { ok: false, error: e.message, files: [], total: 0 };
  }
}

async function listFoldersFull(apiKey) {
  try {
    const res = await httpGet(`${BYSE_BASE}/folder/list?key=${encodeURIComponent(apiKey)}&fld_id=0&files=1`);
    if (res.status === 200 && res.result) {
      const folders = res.result.folders || [];
      return { ok: true, folders };
    }
    return { ok: false, folders: [], error: res.msg || 'Gagal list folder' };
  } catch (e) {
    logger.error('Byse listFoldersFull error', { error: e.message });
    return { ok: false, folders: [], error: e.message };
  }
}

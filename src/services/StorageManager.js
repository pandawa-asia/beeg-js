...
class StorageManager {
  ...
  initialize(dir) {
    try {
      const normalized = (dir && dir !== config.BASE_DIR) ? normalizeDir(dir) : config.BASE_DIR;
  ...
  applyDownloadDir(newDir, chatId = null) {
    try {
      const normalized = (newDir && newDir !== config.BASE_DIR) ? normalizeDir(newDir) : config.BASE_DIR;
      fs.mkdirSync(normalized, { recursive: true });
      this.downloadDir = normalized;
